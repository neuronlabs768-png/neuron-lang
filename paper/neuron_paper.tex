...
\textbf{What this system does not do.} It does not verify that a causal graph is \emph{correct}---only that observed and intervened values are not mixed. It does not prove that temporal annotations are \emph{accurate}---only that future-typed data does not flow into past-only contexts. It does not guarantee uncertainty bounds are \emph{calibrated}---only that confidence is checked before use. It does not type-check across module boundaries. It has not been benchmarked for speed.
...
\textbf{Future work.} Offset-based temporal types (\S\ref{sec:temporal}), cross-module type inference, multi-device and distributed GPU execution, and formal soundness proof.
